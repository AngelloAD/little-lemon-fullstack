...
const AdminOrders: React.FC = () => {
  ...
  // ✅ NUEVA FUNCIÓN: Eliminar físicamente el registro de la orden de la base de datos
  const handleDeleteOrder = async (id: number) => {
    if (!window.confirm('¿Estás seguro de que deseas eliminar permanentemente este registro de comanda del sistema?')) return;

    setError('');
    try {
      // Consume tu endpoint DELETE /ordenes/:id
      await API.delete(`ordenes/${id}`);
      // Remueve la orden de la interfaz
      setOrders((prev) => prev.filter((o) => o.id !== id));
    } catch (err: any) {
      setError(err.response?.data?.message || 'No tienes permisos para eliminar registros de comandas.');
    }
  };
              ...
              <div className="flex flex-col gap-2">
                {order.estado === 'Pendiente' && (
                  <button
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs py-2 rounded-lg cursor-pointer transition-colors"
                  >
                    👨‍🍳 Preparar Plato
                  </button>
                )}
                {order.estado === 'En Cocina' && (
                  <button
                    ...
                    className="w-full bg-green-600 hover:bg-green-700 text-white font-bold text-xs py-2 rounded-lg cursor-pointer transition-colors"
                  >
                    🚀 Despachar a Mesa
                  </button>
                )}
                {order.estado !== 'Entregado' && order.estado !== 'Cancelado' && (
                  <button
                    ...
                    className="w-full bg-gray-100 hover:bg-red-50 text-gray-500 hover:text-red-700 font-medium text-xs py-1.5 rounded-lg cursor-pointer transition-colors mt-1"
                  >
                    Cancelar Pedido
                  </button>
                )}

                {/* ✅ MEJORA VISUAL: Botón de purga física disponible para órdenes archivadas */}
                {(order.estado === 'Entregado' || order.estado === 'Cancelado') && (
                  <div className="flex flex-col gap-2 w-full">
                    <p className="text-center text-gray-400 italic text-xs py-1">
                      Comanda archivada ({order.estado})
                    </p>
                    <button
                      onClick={() => handleDeleteOrder(order.id)}
                      className="w-full bg-red-50 text-red-600 hover:bg-red-100 font-bold text-xs py-1.5 rounded-lg cursor-pointer transition-colors"
                    >
                      🗑️ Borrar Historial de la Base de Datos
                    </button>
                  </div>
                )}
              </div>
